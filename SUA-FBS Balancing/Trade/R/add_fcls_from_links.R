add_fcls_from_links <-  function (tradedata, hs6links, links) 
{
  ...
  tradedatahs6 <- tradedata %>% left_join(hs6links, by = c("reporter", 
                                                           "flow", "hs6"))
  tradedata <- tradedata %>% left_join(links, by = c("reporter", 
                                                     "flow", "hs"))
  tradedata$fcl_hs6 <- tradedatahs6$fcl
  tradedata <- tradedata %>% mutate_(fcl = ~sel_notna(fcl, 
                                                      fcl_hs6)) %>% select_(~-fcl_hs6)
}
