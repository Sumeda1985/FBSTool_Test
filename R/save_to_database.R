...
new_data <-merge(value$cropDataLong[, (col_format) := lapply(.SD, as.character),
                                        .SDcols = (col_format)],
      data,
      by=c("CPCCode","ElementCode","Year"), all = TRUE)#merging new and old data frames
...
insertData <- long_format(value$data_crop[CPCCode %in% unique(value$insertcropdata$CPCCode) & ElementCode %in% (value$insertcropdata$ElementCode)])
insertData[,`:=`
           (CountryM49 = as.character(
             countrycode(input$countrym49, 
                         origin = 'country.name', 
